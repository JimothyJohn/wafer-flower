...
// big end at the wafer side, dropping radially outward by GEAR_F toward
// the mounting wall — meshing a module-matched 45-deg coned
...
function gearSpec(){
  ...
  return {tps:tps, T:T, rp:rp, tip:rp+GEAR_M, root:rp-1.25*GEAR_M,
          tip_front:(rp+GEAR_M)*kb, kb:kb, web_i:Ro-2, m:GEAR_M};
}
...
function buildScene(){
  ...
  for(let j=0;j<=NF;j++){
    const a=-H+2*H*j/NF, rf=toothWave(a,G)*kb, rw=toothWave(a-skew,G), ri=G.web_i;
...
function exportBodies(){
  ...
  const skew=GEAR_F*Math.tan(GEAR_SP*Math.PI/180)/G.rp;
  for(let j=0;j<=NF;j++){
    const a=-H+2*H*j/NF, rf=toothWave(a,G)*kb, rw=toothWave(a-skew,G);
    const c=Math.cos(a), s=Math.sin(a);
    fPos.push(ri*c,ri*s,zBot+GEAR_F, rf*c,rf*s,zBot+GEAR_F,
              ri*c,ri*s,zBot,         rw*c,rw*s,zBot);
  }
