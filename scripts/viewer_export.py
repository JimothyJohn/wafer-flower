...
def main():
    ...
    if a.verify:
        ...
        # the file:// bundle must mirror the committed manifest + STL bytes
        # exactly (both are committed, so bytes ARE comparable)
        jpath = os.path.join(a.out, 'models_data.js')
        if not os.path.exists(jpath):
            stale.append('models_data.js missing — rerun viewer_export.py')
        elif not stale:
            with open(jpath) as f:
                if f.read() != models_data_js(a.out, committed):
                    stale.append('models_data.js does not match the committed '
                                 'manifest/STLs — rerun viewer_export.py')
        print()
        if stale:
    ...
    with open(os.path.join(a.out, 'manifest.json'), 'w') as f:
        ...
        f.write('\n')
    js = models_data_js(a.out, manifest)
    with open(os.path.join(a.out, 'models_data.js'), 'w') as f:
        f.write(js)
    print(f"  models_data.js       base64 bundle for file:// viewing "
          f"({len(js) // 1024} kB)")
...
def models_data_js(outdir, manifest):
    """Single-file base64 bundle of the manifest + every part STL.

    Why it exists: fetch() is blocked under file://, so a double-clicked
    docs/index.html cannot load models/. A <script src> IS allowed under
    file://, so the viewer falls back to this bundle when fetch fails.
    Regenerated (and staleness-gated by --verify) alongside the STLs — the
    STLs stay authoritative and GitHub-previewable; this is a mirror."""
    import base64
    files = {}
    for p in manifest['parts']:
        with open(os.path.join(outdir, p['file']), 'rb') as f:
            files[p['file']] = base64.b64encode(f.read()).decode('ascii')
    return ('// generated by scripts/viewer_export.py — do not edit\n'
            'window.HALO_MODELS='
